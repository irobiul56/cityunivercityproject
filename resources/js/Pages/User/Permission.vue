...
const editPermission = (permission) => {
  permissionForm.id = permission.id; // Set the ID to track the permission being edited
  permissionForm.name = permission.name; // Populate form with existing data
  isModalVisible.value = true;
};

const submitPermission = () => {
    // Determine if we are updating (PUT) or creating (POST)
    const isUpdate = Boolean(permissionForm.id);
    const routeName = isUpdate ? 'permission.update' : 'permission.store';
    const routeParams = isUpdate ? [permissionForm.id] : [];
    const method = isUpdate ? 'put' : 'post'; // Use PUT for update, POST for create

    // Submit the form with appropriate HTTP method
    permissionForm[method](route(routeName, ...routeParams), {
        onSuccess: (page) => {
            isModalVisible.value = false; // Close modal on success
            permissions.value = page.props.permission; // Refresh permissions list
            permissionForm.reset(); // Reset form fields
            permissionForm.id = null; // Reset ID for next add
            const message = isUpdate ? "Permission updated successfully!" : "Permission added successfully!";
            ElMessage.success(message);
        },
        onError: () => {
            ElMessage.error("Failed to submit the permission. Please try again.");
        }
    });
};
...
const deletePermission = (permissionId) => {
    if (confirm("Are you sure you want to delete this data?")) {
        permissionForm.delete(route('permission.destroy', permissionId), {
            onSuccess: (page) => {
                permissions.value = page.props.permission; // Update the permissions list after deletion
                ElMessage.success("Permission deleted successfully!");
            },
            onError: () => {
                ElMessage.error("Failed to delete the permission. Please try again.");
            },
        });
    }
};
...
<template>
    <FrontendLayout>
        ...
        <Head title="Permission page"></Head>
        <div class="container mx-auto">
            <div class="relative mx-4 lg:mx-0 mb-2">
                ...
                <input v-model="searchQuery"
                    ...
                    <el-button class=" ml-3" @click="AddPermission" type="success">Add</el-button>
            ...
            <table class="min-w-full bg-white shadow-md rounded-lg">
                <thead>
                    <tr class="text-left">
                        ...
                        <th class="py-2 px-4 border-b-2">Permission Name</th>
                        <th class="py-2 px-4 border-b-2">Action</th>

                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(permission, index) in filteredPermission" :key="index" class="hover:bg-gray-100">
                        ...
                        <td class="py-2 px-4 border-b flex">
                           ...
                           <!-- Edit button to open modal for editing -->
                            ...
                            <Link @click="deletePermission(permission.id)">
...
>
    <div class="modal-content">
        <!-- Input field for Permission Name -->
        <el-input
    ...
    <template #footer>
        <el-button
            ...
            native-type="submit"
        >
            <span v-if="permissionForm.processing">Submitting...</span>
